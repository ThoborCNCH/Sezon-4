...
import static org.firstinspires.ftc.teamcode.drive.ThoborVARS.Un_Patrat_Intreg;
...
@Autonomous(name = "autonom gyro tehehe", group = "Hanga")
public class Autonom_gyro_tehehe extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (opModeIsActive()) {
            while (opModeIsActive() && !isStopRequested()) {
               if(stop_sa_mor_de_nu == 1) {
                   if (primadata == 0) // that was she said // asta te duce la inele
                   {
//                       moveToPosition(Jumatate_Patrat + (Jumatate_Patrat / 2) + ((Jumatate_Patrat / 2) / 2) - (((Jumatate_Patrat / 2) / 2) / 2) - (((Jumatate_Patrat / 2) / 2) / 2), 0.5);
                       moveToPosition(Un_Patrat_Intreg-(Jumatate_Patrat/2), 0.5);
                       //
                       turnWithGyro(45, -0.2);
                       primadata = 1;
                   }
                   ...
                   if (vreau_sa_mor == 1) {
                       switch (path) {
                           case 1: {
                               ...
                               //
                               moveToPosition(61, 0.4);
                               //

                               strafeToPosition(7, 0.4); // mergem in stanga sa ne cacam in wooble

                               DuWooble(1);

                               strafeToPosition(-24, 0.3); // mergem in stanga sa aruncam

                               moveToPosition(-27.4, 0.3); // merg inapoi la linie
                           ...
                           case 2: {
                               ...
                               //
                               moveToPosition(67, 0.4);
                               turnWithGyro(90, 0.2); //revenire
                               DuWooble(1);
                               strafeToPosition(32.8, 0.4);
                               moveToPosition(-20.2, 0.4);
                               turnWithGyro(90, -0.2); //revenire
                               AruncaAutomat(3);
                           ...
                           case 3: {
                               //
                               moveToPosition(73.6, 0.4);
                               turnWithGyro(180, -0.2); //revenire
                               sleep(400);
                               DuWooble(1);
                               sleep(400);
                               moveToPosition(38.8, 0.4);
                               turnWithGyro(180, 0.2); //revenire
                               strafeToPosition(-19.2, 0.3);
                               AruncaAutomat(3);
    ...
    public void CorectieMiscareDREPT(double power) {
...
/*    public void AruncaAutomat(int nr_inele) {
        opresteMotoare();
    ...
    public void AruncaAutomat(int nr_inele) throws InterruptedException {
        opresteMotoare();
        for (int ajutor_ma_mananca_mama = 1; ajutor_ma_mananca_mama <= nr_inele; ajutor_ma_mananca_mama++) {
            timpAr.reset();
            while (timpAr.time() < 1.2) {
                Arunca(0.7);
            }
